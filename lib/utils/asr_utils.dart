...
Future<String> copyAssetFile(String src, [String? dst]) async {
  ...
  if (await targetFile.exists()) {
    print('[copyAssetFile] File already exists: '
        '[32m$target[0m, size: [33m[1m[4m[7m[5m[31m[0m${await targetFile.length()} bytes[0m');
    return target;
  }

  print('[copyAssetFile] Copying asset: $src to $target');
  final data = await rootBundle.load(src);
  final List<int> bytes = data.buffer.asUint8List(data.offsetInBytes, data.lengthInBytes);
  await targetFile.writeAsBytes(bytes);
  print('[copyAssetFile] Copied asset: $src to $target, size: ${await targetFile.length()} bytes');
