...
class _MeetingDetailScreenState extends State<MeetingDetailScreen> with SingleTickerProviderStateMixin {
  ...
  String _buildCopiedRecords() {
    String ret = '';
    final records = ObjectBoxService().getRecordsByTimeRange(widget.model.startTime, widget.model.endTime);

    for (final record in records) {
      ret += '${DateTime.fromMillisecondsSinceEpoch(record.createdAt!)} ${record.role}: ${record.content}\n';
    }
      ...
      child: BudScaffold(
        ...
        actions: [
          ...
          InkWell(
            splashColor: Colors.transparent,
            highlightColor: Colors.transparent,
            onTapDown: (TapDownDetails details) => _onClickMore(context, details),
            child: BudIcon(
                ...
                children: [
                  Padding(
                    ...
                    child: widget.model.audioPath != null
                        ...
                        : const Text('Cannot find audio file.'),
                  ),
                  BudTabBar(
                    tabs: _tabs,
                ...
                children: [
                  ConvertVoiceWidget(
                    list: ObjectBoxService()
                      ...
                      .map((record) => VoiceModel(
                        name: record.role!,
                        avatar: '',
                        timestamp: DateTime.fromMillisecondsSinceEpoch(
                            record.createdAt!).toTimeFormatString(),
                  ...
                  SummaryWidget(
                    ...
                    onTitleEditComplete: (newTitle) {
                      ObjectBoxService().updateSummaryTitle(widget.model.id, newTitle);
      ...
      child: ListView(
        ...
        children: [
          ...
          ExpansionCard(
              ...
              content: _buildContentForSections(jsonDecode(widget.content!)['sections'])),
          SizedBox(height: 12.sp),
          ExpansionCard(
            icon: AssetsUtil.icon_summary_3,
            title: 'Key points review',
            content: _buildContentForKeyPoints(jsonDecode(widget.content!)['key_points']),
          ),
        ],
      ),
    );
